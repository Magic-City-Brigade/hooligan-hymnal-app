    ...
    return (
      ...
      >
        <View style={{ width: 25, alignItems: "flex-end" }}>
          <BoldText style={{ color: this.props.textColor }}>
            {player.squadNumber}
          </BoldText>
        </View>
        <View style={{ flexDirection: i18n.getFlexDirection() }}>
          ...
          <RegularText style={{ color: this.props.textColor }}>
            ({player.position})
          </RegularText>
        </View>
      </View>
...
class RosterFoePlayers extends React.Component {
  ...
  _renderItem = (item) => {
    return (
      <FoePlayerRow
        item={item}
        onPress={this._handlePressFoeButton}
  ...
  componentDidMount = () => {
    ...
    if (foe.backgroundColor) backgroundColor = foe.backgroundColor;
    if (foe.accentColor) accentColor = foe.accentColor;
    if (foe.textColor) textColor = foe.textColor;
    ...
    let foeImage = (
      <View>
        <MaterialCommunityIcons
          name={Skin.Roster_FoesTabIcon}
          color={DefaultColors.ButtonBackground}
          size={150}
        />
        <BoldText style={{ color: DefaultColors.ColorText, marginBottom: 10 }}>
          {foe.opponent}
        </BoldText>
      </View>
    );
...
export default withUnstated(RosterFoePlayers, {
