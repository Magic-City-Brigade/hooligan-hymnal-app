  ...
  _handlePressPlayer = (player) => {
    if (this.props.route.params.onAttachmentComplete)
      this.props.route.params.onAttachmentComplete({
        ...
        relatedId: player._id,
      });
  };

  render() {
    let filteredPlayers = [];
    if (this.state.search == "") filteredPlayers = this.state.players;
    else filteredPlayers = this.state.players.filter(this.isSearchMatch);
    filteredPlayers.sort(this.comparePlayers);

    return (
      <View style={{ flex: 1 }}>
        <BoldText style={{ textAlign: "center" }}>
          {i18n.t("screens.postattachmentselectplayer.selectplayer")}
        </BoldText>
        <View
        ...
        <FlatList
          ...
          renderItem={(item) => {
            return (
              <PlayerRow player={item.item} onPress={this._handlePressPlayer} />
            );
          }}
...
export default withUnstated(PostAttachmentSelectPlayer, {
