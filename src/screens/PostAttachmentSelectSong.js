...
class SongRow extends React.Component {
  render() {
    ...
    if (song.referenceTitle)
      referenceDisplay = (
        <RegularText style={styles.reference}>
          {song.referenceTitle}
        </RegularText>
      );
            ...
            parse={[
              {
                pattern: /(\*)(.*?)\1/,
                style: styles.bold,
                renderText: this._renderFormatted,
              },
              {
                pattern: /(_)(.*?)\1/,
                style: styles.italic,
...
class PostAttachmentSelectSong extends React.Component {
  ...
  render() {
    let filteredSongs = [];
    if (this.state.search == "") filteredSongs = this.state.songs;
    else filteredSongs = this.state.songs.filter(this.isSearchMatch);
    filteredSongs.sort(this.compareSongs);

    return (
      <View style={{ flex: 1 }}>
        <BoldText style={{ textAlign: "center" }}>
          {i18n.t("screens.postattachmentselectsong.selectsong")}
        </BoldText>
        <View
        ...
        <FlatList
          ...
          data={filteredSongs}
          renderItem={(item) => {
            return <SongRow song={item.item} onPress={this._handlePressSong} />;
          }}
...
export default withUnstated(PostAttachmentSelectSong, {
