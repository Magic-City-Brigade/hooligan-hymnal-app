...
class SongRow extends React.Component {
  render() {
    ...
    let capoSignal;
    if (song.capoSignal) capoSignal = "📢: " + song.capoSignal;
      ...
      playDisplay = (
        ...
        >
          <MaterialCommunityIcons
            name={"play-circle"}
            style={{ color: Skin.Home_SocialButtons }}
          />
        </View>
      );
...
class SongbookContents extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      prevProps.globalData.state.songbookContents !=
      this.props.globalData.state.songbookContents
    )
      this.setData();
  }
  ...
  setData() {
    if (!this.state.ToCData)
      this.props.globalData.computeSongbook(() =>
        this.setState({ ToCData: this.props.globalData.state.songbookContents })
      );
  }
...
export default withUnstated(SongbookContents, {
