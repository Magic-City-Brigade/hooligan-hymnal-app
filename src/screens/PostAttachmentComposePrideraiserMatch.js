...
export default class PostAttachmentComposePrideraiserMatch extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      if (campaign.hasOwnProperty("id")) {
        ...
        let source = "";
        if (
          Settings.PostAttachmentComposePrideraiserMatch_AnalyticsSourcePrefix
        )
          source +=
            Settings.PostAttachmentComposePrideraiserMatch_AnalyticsSourcePrefix +
            "-";
        source += moment(new Date()).format(
          Settings.PostAttachmentComposePrideraiserMatch_AnalyticsSourceDateFormat
        );
        if (
          Settings.PostAttachmentComposePrideraiserMatch_AnalyticsSourceSuffix
        )
          source +=
            "-" +
            Settings.PostAttachmentComposePrideraiserMatch_AnalyticsSourceSuffix;
  ...
  render() {
    ...
    if (!this.state.loadedCampaign && !this.state.error) {
      loader = (
        <View style={{ flexDirection: i18n.getFlexDirection(), width: "100%" }}>
          <ActivityIndicator
            animating={true}
            color={
              Settings.PostAttachmentComposePrideraiserMatch_ActivityIndicator
            }
          />
          <RegularText style={{ color: DefaultColors.ColorText }}>
            Loading
          </RegularText>
        </View>
      );
    ...
    return (
      <ScrollView style={styles.container}>
        <BoldText style={{ textAlign: "center" }}>
          {i18n.t(
            "screens.postattachmentcomposeprideraisermatch.prideraisermatch"
          )}
        </BoldText>
        {loader}
        ...
        <BoldText>
          {i18n
            .t("screens.postattachmentcomposeprideraisermatch.howmanygoals")
            .replace(
              "%goal_name_plural%",
              this.state.campaign.goal_name_plural
            )}
        </BoldText>
        <TextInput
          ...
          onChangeText={(text) =>
            this.setState({ goalCount: parseInt(text, 10) })
          }
          keyboardType={"numeric"}
        />
        <BoldText>
          {i18n.t("screens.postattachmentcomposeprideraisermatch.analytics")}
        </BoldText>
        <TextInput
          style={styles.sourceInput}
          value={this.state.source}
          placeholder={i18n.t(
            "screens.postattachmentcomposeprideraisermatch.source"
          )}
        ...
        <BigButton
          ...
          buttonStyle={
            !this.state.loadedCampaign
              ? { backgroundColor: "gray", marginBottom: 15 }
              : { marginBottom: 15 }
          }
              ...
              this.props.route.params.onAttachmentComplete({
                ...
                data: {
                  ...
                  campaign: {
                    ...
                    supporters_group: {
                      name: this.state.campaign.supporters_group
                        ? this.state.campaign.supporters_group.name
                        : this.state.campaign.supporter_group.name,
                    },
                  },
                },
              });
