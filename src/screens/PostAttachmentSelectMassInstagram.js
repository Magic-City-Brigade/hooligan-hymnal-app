...
class PostAttachmentSelectMassInstagram extends React.Component {
  state = {
    search: "",
    rosters: [],
  };
  ...
  compareRosters = (a, b) => {
    ...
    if (b.season > a.season) return false;
  ...
  render() {
    let filteredRosters = [];
    if (this.state.search == "") filteredRosters = this.state.rosters;
    else filteredRosters = this.state.rosters.filter(this.isSearchMatch);
    filteredRosters.sort(this.compareRosters);

    return (
      <View style={{ flex: 1 }}>
        <BoldText style={{ textAlign: "center" }}>
          {i18n.t("screens.postattachmentselectmassinstagram.selectroster")}
        </BoldText>
        <View
          style={{ flexDirection: i18n.getFlexDirection(), borderWidth: 1 }}
        >
          <TextInput
            style={styles.textInput}
            placeholder={i18n.t(
              "screens.postattachmentselectmassinstagram.search"
            )}
        ...
        <FlatList
          ...
          renderItem={(item) => {
            return (
              <RosterRow roster={item.item} onPress={this._handlePressRoster} />
            );
          }}
...
export default withUnstated(PostAttachmentSelectMassInstagram, {
